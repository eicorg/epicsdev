...
__version__ = 'v0.0.1 26-08-10'# AI generated by Copilot
...
def myPVDefs():
    ...
    pvDefs = [
        ...
        ['trigState', 'Current trigger state', '?', {SCPI: ':TRIGger:STATus'}],
...
def trigger_is_detected():
    ...
    ts = timer()
    try:
        trigState = scopeCmd(':TRIGger:STATus?')
    except VisaIOError:
        handle_exception('in trigger_is_detected')
        return False

    trigState = trigState.upper()
    edev.publish('trigState', trigState, IF_CHANGED)
    ElapsedTime['trigger_detection'] = round(timer() - ts, 6)

    if trigState.startswith('TRIG') or trigState.startswith('TD'):
        C_.trigTime = time.time()
        return True

    # In AUTO sweep, acquire periodically even if explicit trigger event is not reported.
    return str(edev.pvv('trigMode')).upper().startswith('AUTO')
