...
__version__ = 'v0.0.6 2026-05-20'# Wait for field to stabilize, when (1 - last/current) < threshold
#TODO: Test wait_for_field_to_stabilize()
...
def session():
    ...
    while EventScan.wait():
        ...
        EventScan.clear()
        if C_.measure == Stop:
            return ('Measurement mode changed to Stop')
        if C_.measure != Scan:
        ...
        for lcc in setPointRange('LCC'):
            ...
            for icc1 in setPointRange('ICC1'):
                ...
                for icc2 in setPointRange('ICC2'):
                    ...
                    wait_for_field_to_stabilize()

                    IFace.put(C_.magnetometer+'update', C_.steps) # trigger measurement
                    perf['update'] += timer() - ts
                    C_.steps += 1
                    ts = timer()
                    C_.lastField = IFace.get(C_.magnetometer+'field')
                    perf['field'] += timer() - ts
                    C_.writer.writerow([f'{pos:.5g}', f'{lcc:.5g}', f'{icc1:.5g}', f'{icc2:.5g}', f'{C_.lastField:.5g}'])
        printi(f'Recorded {C_.steps} points. You can change position and scan again')
        set_coils_to_minimum()

    return f'Session completed with {C_.steps} steps'

def wait_for_field_to_stabilize(timeout=5.0, threshold=0.01):
    """Wait for the field to stabilize by checking if the change in field is below a threshold for a certain duration
    """
    time.sleep(C_.sleepTime) # wait for magnet field to stabilize

    start_time = time.time()
    #IFace.put(C_.magnetometer+'update', C_.steps)
    #C_.lastField = float(IFace.get(C_.magnetometer+'field'))
    while time.time() - start_time < timeout:
        #time.sleep(0.1) # wait a bit before checking again
        IFace.put(C_.magnetometer+'update', C_.steps)
        current_field = float(IFace.get(C_.magnetometer+'field'))
        ts = time.time()
        if C_.lastField != 0.:
            if abs(1. - C_.lastField/current_field) < threshold:
                #printi(f'field stabilized from {C_.lastField:.3f} to {current_field:.3f} in {ts - start_time:.3f} s')
                return True # field is stable
        C_.lastField = current_field
    printw('Field did not stabilize within timeout')
    return False

def set_coils_to_minimum():
    print('Setting all coils to minimum.')
    for coilConfig in C_.setPoints.values():
         IFace.put(coilConfig[0],coilConfig[1])

def zero_coils():
    print('Setting all coils to 0.')
    for coilConfig in C_.setPoints.values():
         IFace.put(coilConfig[0], 0.)
