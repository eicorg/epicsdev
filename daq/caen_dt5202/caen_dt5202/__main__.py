...
__version__ = 'v1.0.4 2026-08-05'# added run and runStartTime PVs
...
def my_pv_defs():
    ...
    SET = "setter"
    pvdefs = [
['instrument','Instrument instance', C_.pargs.instance],
#['ipAddress','IP address of the DT5202 digitizer', [C_.pargs.ipAddress]],
['run','Run number', 0],
...
class Dt5202PVServer:
    def update_b0channels(self, parsed: dict) -> None:
        epicsdev.publish('run', C_.run)
        epicsdev.publish('runStartTime', C_.runStartTime)
        for key in parsed:
...
def parse_run_file(path: Path):
    ...
    C_.runStartTime = dt_obj.strftime('%y-%m-%d %H:%M:%S')
...
def process_one_file(file_path: Path, out_dir: Path, pva: Dt5202PVServer):
    ...
    C_.run = int(file_path.stem.split('_')[0][3:])
    print(f"Processing file: {file_path}, run: {C_.run}, runStartTime: {C_.runStartTime}")
